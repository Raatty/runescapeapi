...
class Highscores:
    ...
    def _fetch(self, rsn, type, skill_count: int):
        ...
        for index, lvl in enumerate(levels):
            lvl_split = lvl.split(",")
            yield {
                "level": int(lvl_split[1]),
                "xp": int(lvl_split[2]),
                "rank": int(lvl_split[0]),
                "id": index,
                "name": list(self.SKILL_NAMES.keys())[index],
            }
    ...
    def _calculate_combat(self):
        if self.skill_count == 27:
            ...
            first = 0.325 * max(
                self.skill("Attack") + self.skill("Strength"),
                1.5 * self.skill("Magic"),
                1.5 * self.skill("Ranged"),
            )
    ...
    def __init__(self, rsn: str, type_: str = None):
        ...
        if type_ is None:
            ...
            if type_ in ["hiscore", "hiscore_ironman", "hiscore_hardcore_ironman"]:
                self.type = type_
                self.skill_count = 27
            elif type_ in [
                "hiscore_oldschool",
...
class Player:
    ...
    CLAN_AND_TITLE_URL = (
        BASE_URL
        + "m=website-data/playerDetails.ws?names=%5B%22{}%22%5D&callback=jQuery000000000000000_0000000000&_=0"
    )
    ...
    def _fetch_runemetrics(self):
        ...
        try:
            ...
            self.profile["stats"] = sorted(
                [
                    dict(
                        i, **{"name": list(Highscores.SKILL_NAMES.keys())[i["id"] + 1]}
                    )
                    for i in metrics["skillvalues"]
                ],
                key=lambda x: Highscores.SKILL_NAMES[x["name"]],
            )

    def _fetch_quests(self):
        self.profile["quest_list"] = requests.get(
            self.RUNE_METRICS_QUESTS_URL.format(self._rsn)
        ).json()["quests"]
...
class Clan:
    ...
    def __init__(self, clan: str):
        self.name = clan
        self.members = (
            requests.get(self.CLAN_MEM_URL.format(clan.replace(" ", "+")))
            .content.replace(b"\xa0", b" ")
            .decode("ascii")
            .split("\n")[1:-1]
        )
...
class Beasts:
    ...
    def by_letter(letter: str):
        ...
        """
        BEAST_LETTER_URL = (
            BASE_URL + "m=itemdb_rs/bestiary/bestiaryNames.json?letter={}"
        )
    ...
    def by_category(cat):
        ...
        """
        BY_CATEGORY_URL = (
            BASE_URL + "m=itemdb_rs/bestiary/slayerBeasts.json?identifier={}"
        )
        if type(cat) == int:
    ...
    def by_weakness(weakness):
        ...
        """
        BY_WEAKNESS_URL = (
            BASE_URL + "m=itemdb_rs/bestiary/weaknessBeasts.json?identifier={}"
        )
    ...
    def by_level(lower_: int, upper_: int):
        ...
        """
        BY_LEVEL_URL = (
            BASE_URL + "m=itemdb_rs/bestiary/levelGroup.json?identifier={}-{}"
        )
